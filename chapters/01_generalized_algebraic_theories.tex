...
% TODO: if only one notion included change this, if too many create a new section 
First we will start by defining some concepts key to understanding the
structure of this categories.

Our notion of \textbf{tree} consist of trees with a unique least element.
Let $\Theta$ be a tree. If $A$ is a node of $\Theta$
we will write $A \in \Theta$, confusing the tree with its set of nodes.
Given two nodes $A, B \in \Theta$ such that $B$ succeeds $A$,
we write $A \vartriangleleft B$ in $\Theta$.
The least node of a tree will be denoted as $1$.
If we have a node $A$ of $\Theta$ different from $1$, then 
there exists a unique $n \geq 0$ and a unique sequence
$A_1, \dots, A_n$ such that 
$1 \vartriangleleft A_1 \vartriangleleft A_2 \vartriangleleft \dots \vartriangleleft A_n \vartriangleleft A$
in $\Theta$.

We call the \textbf{tree of families} to the following tree structure
over the collection of sets, families of sets, families of families of sets, \dots
where a set $A$ precedes all families of sets indexed by $A$, 
a family of sets $B$ precedes all families of families of sets 
indexed by $B$, \dots % TODO: is this the correct way of writing this?
If $A_1$ is a set, and $A_2$ is a $A_1$-indexed family of sets
then for $a_1 \in A_1$ we write $A_2(a_1)$ for the value of
the family $A_2$ at $a_1$.
If we have $1 \vartriangleleft A_1 \vartriangleleft A_2 \vartriangleleft \dots \vartriangleleft A_n \vartriangleleft A$
in the tree of families, then for all $a_1 \in A_1$, \dots
and for all $a_n \in A_n(a_1, \dots, a_{n-1})$, $A(a_1, \dots, a_n)$
is a set.

% TODO: continue

Now we will define the category of contexts and realizations of a theory $U$.
