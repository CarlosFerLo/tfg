...
\begin{proof}
    ...
    \begin{itemize}
        ...
        \begin{itemize}
            ...
            \item[\ref{rule:forall-elim}] $\Gamma \vdash \Proof((\forall x\colon B)p) = (\Pi x\colon B)\Proof(p)$. If $A = \Proof((\forall x\colon B)p)$, it is Case 2. If $A \equiv (\Pi x\colon B)\Proof(p)$, it is Case 3.
        \end{itemize}
    \end{itemize}
\end{proof}
...
\begin{proof}
    ...
    \begin{itemize}
        ...
        By inner induction on the derivation of term equality. If derived from $\Gamma \vdash t\colon A$, it is shorter (apply outer induction hypothesis). If derived by \ref{rule:trans-obj}, the companion term-equality lemma determines the form. If derived by \ref{rule:beta-rule} or \ref{rule:eta-rule}, $t$ maps directly to Case 3 or Case 2, respectively.
    \end{itemize}
\end{proof}